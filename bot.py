...
def get_referrer(init_data):

    """
    Получает ref_ID из:
    https://t.me/GiftsUpp_bot?start=ref_ID
    """
...
def api_me():
    ...
    referrer_id = get_referrer(
        init_data
    )


    # Создаём пользователя
    # и начисляем рефералу 0.85 ⭐
...
который пришёл по твоей ссылке,
начисляется 0.85 ⭐.

Один пользователь может
быть засчитан только один раз.
